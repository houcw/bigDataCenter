  ...
  .sl-c-discern-log{
    ...
    .el-table{
      ...
      .img{
        width: 96px;
        height: 96px;
        background-image: url("http://b-ssl.duitang.com/uploads/item/201502/03/20150203144840_zGmdm.jpeg");
  ...
  <div class="sl-c-discern-log">
    ...
    <div class="sl-header clrfix">
      ...
      <div class="sl-item sl-item-status">
        ...
        <div class="fl">
          <el-input v-model="nameOrMobile" clearable placeholder="请输入客人名称"></el-input>
        </div>
      </div>
      <!--认证结果-->
      <div class="sl-item sl-item-status">
      ...
      <div class="sl-item sl-item-status">
        ...
        <div class="fl">
          <el-select v-model="checkInResult" clearable placeholder="请选择">
            <el-option
      ...
      <div class="table">
        <template>
          <el-table :data="tableData" style="width: 100%" border>
            <el-table-column prop="hotelName" label=""></el-table-column>
            <el-table-column prop="building,floor" label="楼栋楼层">
            ...
            <el-table-column prop="logTime" label="认证时间"></el-table-column>
            ...
            <el-table-column prop="imgUrl" label="身份证头像">
              <template slot-scope="scope">
                <div class="img" :style="{ 'background-image': 'url(' + scope.row.imgUrl + ')' }"></div>
              </template>
            </el-table-column>
            <el-table-column prop="imgUrl" label="实拍图">
              <template slot-scope="scope">
                <div class="img" :style="{ 'background-image': 'url(' + scope.row.registerImgUrl  + ')' }"></div>
              </template>
            </el-table-column>
            <el-table-column prop="threshold" label="相似度"></el-table-column>
            <el-table-column prop="discernResult" label="认证结果">
              <template slot-scope="scope">
            ...
            <el-table-column prop="checkInResult" label="入住结果">
              <template slot-scope="scope">
                <span :style="scope.row.checkInResult==='Y'?{color:'rgba(68, 68, 68, 1)'}:{color:'rgba(255, 84, 84, 1)'}">
                  {{scope.row.checkInResult==='Y'?'成功':'失败'}}
                </span>
              </template>
            </el-table-column>
...
<script>
  export default {
    ...
    methods:{
      onload(){
        // 入住机统计数据
        this.$store.dispatch('occupmng/registerStatistics').then((res)=>{
